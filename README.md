...
When any pattern matches, placeholders such as `{file_name}`, `{directory_name}`, `{folder_and_file}`, `{relative_file_path}`, and `{full_directory_name}` are cleared so names do not appear in Discord; **`{file_extension}`** and **`{lang}`** (language icon keys) still resolve. **Workspace-level ignore** (`vscord.ignore.workspaces`) is unchanged and still replaces **details/state** with `vscord.ignore.workspacesText` (e.g. “Private Project”) for the whole workspace.
